...
pub fn event_derive(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    for f in indexed_fields.iter() {
        ...
        option_fields.push(quote! { #ident: Option<oasis_std::types::H256> });
        topic_setters.push(quote! {
            fn #setter_ident(&mut self, #ident: &#ty) -> &mut Self {
                self.#ident = Some(oasis_std::types::H256::from_slice(
    ...
    proc_macro::TokenStream::from(quote! {
        mod #impl_wrapper_ident {
            ...
            impl #topics_struct_ident {
                ...
                pub fn hash(&mut self) -> Vec<oasis_std::types::H256> {
                    vec![ #(#topic_getters),* ]
                }
            }
            ...
            impl oasis_std::exe::Event for #event_name  {
                ...
                fn emit(&self) {
                    ...
                    let topics = std::iter::once(#event_name_topic)
                        ...
                        .collect::<Vec<_>>();
                    oasis_std::ext::log(&topics, &serde_cbor::to_vec(self).unwrap());
                }
            }
        }
    })
